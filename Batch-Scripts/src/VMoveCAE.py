...
class VmoveCaeFrame(wx.Frame):
    ...
    def addToolBar(self):
        ...
        # self.tools_toolbar.AddTool(self.Id_Sort, NotSorted.GetBitmap(),
            ...
            if (fext == '.rst' or fext == '.rth') and self.ansys_input_commands_prompt_on:
                ...
                if load_ds_dat:
                    ...
                    ds_dat_dlg = wx.FileDialog(self, 
                            ...
                            #style=wx.OPEN | wx.MULTIPLE | wx.CHANGE_DIR)
                            style=wx.OPEN | wx.FD_FILE_MUST_EXIST | wx.FD_CHANGE_DIR)
        ...
        if 1 == len(selections):
            for item in selections:
                ...
                if pp_item and pp_item == self.part_root or p_item == self.part_root:
                    ...
                    self.manager.GetPane('Cut-Section').Hide()
                    self.manager.GetPane('Iso-Surface').Hide()
                #elif self.flowlines_root == item.GetParent():
                #    self.flowline_apply_button.Enable(True)
                #    self.manager.GetPane('Iso-Surface').Hide()
                #    self.manager.GetPane('Cut-Section').Hide()
                #    self.manager.GetPane('Iso-Volume').Hide()
                #    self.manager.GetPane('Flow Lines').Show()
    ...
    #     if parent_item == self.egroup_root:
    #         item_name = item.GetText()
    #         item_data = item.GetData()
    #         if item_data > 0:
    #             if event.IsChecked() != self.result_group_checkbox.GetValue():
    #                 self.result_group_checkbox.SetValue(event.IsChecked())
    #         else:
    #             if event.IsChecked() != self.no_result_group_checkbox.GetValue():
    #                 self.no_result_group_checkbox.SetValue(event.IsChecked())
#---------PART--------------------------------------------------------------------------------------------
...
#----------PART-------------------------------------------------------------------------------------------
    ...
    #     self.branches = []
    #     self.grping_tree = self.part_tree
...
#-----------------------------------------------------------------------------------------------------
    ...
    def AddPart(self, element):
        ...
        if not self.part_root:
            # self.part_root = self.part_tree.AddRoot('Geometry and Features',1)
            self.part_root = self.part_tree.AddRoot('Geometry',1)
            # self.part_tree.AppendItem(self.part_root, 
            #                 'Geometry', 1)
    ...
    def UpgradeOdbWizard(self, odb_file, odb_version):
        ...
        wizard = wiz.Wizard(self, self.Id_Wizard, 'Upgrade ODB file',
                    WizardIcon.GetBitmap())
        wizard_message = self.GetOdbUpgradeWizardMessage(odb_file,
                odb_version)
        
        wizard_page = wiz.WizardPageSimple(wizard)
    ...
    def ClearAllUI(self):
        ...
        self.classifier_node_names = []
        if self.part_root:
            self.part_tree.Delete(self.part_root)#DeleteChildren(self.part_root)
        # if self.resprop_root:
        #     self.result_tree.Delete(self.resprop_root)
        if self.res_root:
    ...
    def AddFeatureEdges(self):
        if not self.feature_edges_root:
            if not self.features_root:
                self.features_root = self.part_tree.AppendItem(
                                self.part_root, 'Features', 1)
    ...
    def AddCutSection(self, cutsec_name):
        if not self.features_root:
            self.features_root = self.part_tree.AppendItem(self.part_root, 
                            'Features', 1)
    ...
    def OnReset(self, event):
        for item in self.interface_info.enabled_grouping_option_interfaces:
            self.tab_panels[item].SetGroupingType(
                    self.grouping_types.index(self.interface_grouping[item]))
